...
class OtelLogger(BaseLogger):
    def __init__(self):
        ...
        logger_provider.add_log_record_processor(
            BatchLogRecordProcessor(AzureMonitorLogExporter.from_connection_string(
    ...
    def log_new_wrapper(
        ...
        try:
            log_data = json.dumps(
                {
                    ...
                    "session_id": self.session_id,
                    # "json_state": json.dumps(init_args), # This is commented out because it logs sensitive data like API keys
                    "timestamp": get_current_ts(),
                    "thread_id": thread_id,
                }
            )
            ...
            self.logger.error(
                f"[otel_logger] Failed to log new wrapper event {e}")
    ...
    def log_new_client(
        ...
        try:
            ...
            self.logger.error(
                f"[otel_logger] Failed to log new client instance {e}")
    ...
    def log_function_use(self, source: Union[str, Agent], function: F, args: Dict[str, Any], returns: Any) -> None:
        ...
        try:
            ...
            self.logger.error(f"[otel_logger] Failed to log fucntion use {e}")
